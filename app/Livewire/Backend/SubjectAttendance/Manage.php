...
namespace App\Livewire\Backend\SubjectAttendance;
...
class Manage extends Component
{
    ...
    public function onSubjectChange($value)
    {
        $this->subject_id = $value ?: null;
        $this->attendances = [];
    ...
    protected function prefillExisting(): void
    {
        if (! $this->school_class_id || ! $this->subject_id || ! $this->attendance_date) {
            return;
        }

        $existing = SubjectAttendance::query()
    ...
    {
        if (! $this->school_class_id || ! $this->subject_id || ! $this->attendance_date) return;
        ...
        // get student roll number for better message
        $student = Student::find($studentId);

        $this->dispatch('notify', [
            'type' => 'success',
            'message' => "Attendance updated for Student ID {$studentId} (Roll: {$student->roll_number})."
        ]);
    }
    ...
    public function render()
    {
        ...
        return view('livewire.backend.subject-attendance.manage', [
            'students' => $students,
            'classes' => SchoolClass::all(),
            'shifts' => Shift::all(),
            'departments' => Department::all(),
            'subjects' => ClassSubjectAssign::query()
                ->when($this->school_class_id, fn($q) => $q->where('school_class_id', $this->school_class_id))
                ->when($this->class_section_id, fn($q) => $q->where('class_section_id', $this->class_section_id))
                ->when($this->department_id, fn($q) => $q->where('department_id', $this->department_id))
                ->with('subject') // eager load subject
                ->get()
                ->pluck('subject', 'subject_id'), // pluck subject models keyed by ID

<div>
    <div class="row">
        ...
        <div class="col-md-4 mb-3">
            <div class="card">
                <div class="card-header bg-primary">
                    <div class="card-title m-0 text-white">{{ $incomeHeadId ? 'Edit' : 'Add' }} Income Head</div>
                </div>
                <div class="card-body">
                    <form wire:submit.prevent="save">
                        ...
                        <button class="btn btn-primary" type="submit">{{ $incomeHeadId ? 'Update' : 'Save' }}</button>
        ...
        <div class="col-md-8">
            <div class="card">
                <div class="card-header bg-primary">
                    <div class="card-title m-0 text-white">Income Head List</div>
                </div>
                <div class="card-body">
                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered table-hover table-striped mb-0">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>{{ ($incomeHeads->currentPage() - 1) * $incomeHeads->perPage() + $loop->iteration }}</td>
                                    <td>{{ $head->name }}</td>
                                    <td>{{ $head->description }}</td>
                                    <td>
                                        <button class="btn btn-sm btn-primary" wire:click="edit({{ $head->id }})"><i class="ri-edit-line"></i></button>
                                        <button class="btn btn-sm btn-danger" wire:click="confirmDelete({{ $head->id }})" data-bs-toggle="modal" data-bs-target="#deleteModal"><i class="ri-delete-bin-line"></i></button>
                                    </td>
                                </tr>
                                @empty
                                <tr>
                                    <td colspan="4" class="text-center">No income heads found.</td>
                                </tr>
                                @endforelse
                            </tbody>
                        </table>
